import { BookOpen, Brain, ClipboardList, Database, Eye, History, Info, Leaf, Palette, PawPrint, RotateCcw, ShieldCheck, Sparkles, Type, UserRound } from "lucide-react";
import { useState } from "react";
import Mascot, { Flower } from "../components/Mascot";
import { Button, Card, DemoBadge, Toggle } from "../components/ui";
import { useChatMemory } from "../hooks/useChatMemory";
import type { ChatHistoryApi } from "../hooks/useChatHistory";
...
export default function ProfilePage({ profile, preferences, setProfile, setPreferences, navigate, notify, isRegistered, onResetRegistration, screenings, deviceId, chatHistory, emotionCalendar }: { profile: DemoProfile; preferences: Preferences; setProfile: (profile: DemoProfile) => void; setPreferences: (value: Preferences) => void; navigate: (view: MainView) => void; notify: (text: string) => void; isRegistered: boolean; onResetRegistration: () => void; screenings: ScreeningsState; deviceId: string; chatHistory: ChatHistoryApi; emotionCalendar: EmotionCalendarApi }) {
  ...
  return <div className="page profile-page">
    ...
    <div className="profile-layout"><aside><Card className="profile-card"><Mascot id={profile.mascot} size="medium" /><h2>{profile.name}</h2><p>{profile.city}</p><span className="profile-status"><span /> {isRegistered ? "Registro completado" : "Modo invitado"}</span></Card><Card className="privacy-card"><ShieldCheck size={24} /><h3>Estado de los datos</h3><ul><li><span>Correo y contraseña</span><strong>No se guardan</strong></li><li><span>Expediente clínico</span><strong>No existe</strong></li><li><span>Temas de conversación</span><strong>{preferences.rememberConversations ? "Solo el tema, en este dispositivo" : "No se guardan"}</strong></li><li><span>Historial completo del chat</span><strong>{preferences.saveChatHistory ? "En este dispositivo" : "No se guarda"}</strong></li><li><span>Calendario emocional</span><strong>{emotionCalendar.entries.length ? `${emotionCalendar.entries.length} etiquetas, sin texto` : "Sin registros"}</strong></li><li><span>Resultados de tamizaje</span><strong>{screenings.results.length > 0 ? "En este dispositivo" : "Sin resultados guardados"}</strong></li><li><span>Perfil y ajustes</span><strong>{isRegistered ? "En este dispositivo" : "Solo esta sesión"}</strong></li></ul></Card></aside><div className="settings-stack">
      ...
      <Card><div className="settings-title"><History size={21} /><div><h2>Historial de chats</h2><p>Guarda tus conversaciones completas para poder reabrirlas después, desde el panel del chat.</p></div></div><Toggle checked={preferences.saveChatHistory} onChange={(value) => { update("saveChatHistory", value); if (!value) chatHistory.clear(); }} label="Guardar mis conversaciones" description="Solo en este navegador — nunca se envía al servidor. Desactivarlo borra lo ya guardado." />{preferences.saveChatHistory && chatHistory.sessions.length > 0 && <Button variant="ghost" onClick={() => { chatHistory.clear(); notify("Se borró el historial de conversaciones."); }}><RotateCcw size={16} /> Borrar {chatHistory.sessions.length} {chatHistory.sessions.length === 1 ? "conversación guardada" : "conversaciones guardadas"}</Button>}</Card>
      <Card><div className="settings-title"><Sparkles size={21} /><div><h2>Calendario emocional</h2><p>Guarda fechas y etiquetas tentativas detectadas en el chat, nunca el texto que escribiste.</p></div></div>{emotionCalendar.entries.length > 0 ? <Button variant="ghost" onClick={() => { emotionCalendar.clear(); notify("Se borró el calendario emocional."); }}><RotateCcw size={16} /> Borrar {emotionCalendar.entries.length} {emotionCalendar.entries.length === 1 ? "registro emocional" : "registros emocionales"}</Button> : <p className="fine-print">Aún no hay registros. Aparecerán al expresar una emoción en el chat.</p>}</Card>
      <Card>
