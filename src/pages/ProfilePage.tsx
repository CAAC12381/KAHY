...
export default function ProfilePage({ profile, preferences, setProfile, setPreferences, navigate, notify, isRegistered, onResetRegistration, screenings, deviceId, chatHistory }: { profile: DemoProfile; preferences: Preferences; setProfile: (profile: DemoProfile) => void; setPreferences: (value: Preferences) => void; navigate: (view: MainView) => void; notify: (text: string) => void; isRegistered: boolean; onResetRegistration: () => void; screenings: ScreeningsState; deviceId: string; chatHistory: ChatHistoryApi }) {
  ...
  return <div className="page profile-page">
    ...
    <div className="profile-layout"><aside><Card className="profile-card"><Mascot id={profile.mascot} size="medium" /><h2>{profile.name}</h2><p>{profile.city}</p><span className="profile-status"><span /> {isRegistered ? "Registro completado" : "Modo invitado"}</span></Card><Card className="privacy-card"><ShieldCheck size={24} /><h3>Estado de los datos</h3><ul><li><span>Correo y contraseña</span><strong>No se guardan</strong></li><li><span>Expediente clínico</span><strong>No existe</strong></li><li><span>Temas de conversación</span><strong>{preferences.rememberConversations ? "Solo el tema, en este dispositivo" : "No se guardan"}</strong></li><li><span>Historial completo del chat</span><strong>{preferences.saveChatHistory ? "En este dispositivo" : "No se guarda"}</strong></li><li><span>Resultados de tamizaje</span><strong>{screenings.results.length > 0 ? "En este dispositivo" : "Sin resultados guardados"}</strong></li><li><span>Perfil y ajustes</span><strong>{isRegistered ? "En este dispositivo" : "Solo esta sesión"}</strong></li></ul></Card></aside><div className="settings-stack">
